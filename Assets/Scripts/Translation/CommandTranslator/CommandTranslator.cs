...
namespace T2G.Assistant
{
    ...
    public class CommandTranslator : TranslatorBase
    {
        ...
        private (string pattern, string key)[] _RBP_Rules = 
        {
            ...
            (@"^(?:\w+\s+)?(create|add)\s+(?:a\s+|an\s+)?(?:new\s+)?(?<desc>[\w\s\-]+?)(?:\s+(?:called|named|name|with\s+the\s+name)\s+(?<name>""[^""]+""|[\w\s\-]+))?(?:\.)?$", T2G.Actions.create_object),
            ...
            (@"^add\s+(behavior|behaviour|component)\s+(?<name>[^\s]+)\s+to\s+(?<object>""[^""]+""|'[^']+'|[\w\- ]+)\s*$", "add_behavior"),
